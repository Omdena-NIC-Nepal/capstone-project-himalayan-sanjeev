...
def load_prediction_models():
    ...
    try:
        ...
        if os.path.exists(temp_model_path) and os.path.exists(precip_model_path):
            ...
            error_msg = f"""Model files not found at expected path: {models_dir}
Files in models/: {os.listdir(models_dir) if os.path.exists(models_dir) else 'models/ folder not found'}
"""
            return None, None, error_msg

    except Exception as e:
        error_msg = f"Exception while loading models: {str(e)}"
        return None, None, error_msg
